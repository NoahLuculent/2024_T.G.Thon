document.addEventListener('DOMContentLoaded', () => {
    ...
    function toggleFavorite(star) {
        ...
        if (star.classList.contains('favorited')) {
            ...
            const beforeLetter = letters.find(l => parseInt(l.dataset.originalIndex) === originalIndex - 1);
            if (beforeLetter) {
    ...
    function sortLetters(order) {
        if (order === 'oldest') {
            return letters.sort((a, b) => {
                const timeA = parseExtendedTime(a.querySelector('.letter-timer').getAttribute('data-time'));
                const timeB = parseExtendedTime(b.querySelector('.letter-timer').getAttribute('data-time'));
    ...
    function parseExtendedTime(timeString) {
        ...
        return (((months * 30 + days) * 24 + hours) * 3600) + (minutes * 60) + seconds;
    }
    ...
    function updateTimers() {
        letters.forEach(letter => {
            ...
            let remainingTime = parseExtendedTime(timerElement.getAttribute('data-time'));
            ...
            if (remainingTime > 0) {
                ...
                if (!timerElement.classList.contains('revealed')) {
                    ...
                    timerElement.innerHTML = "00이 보낸 편지가 도착했습니다";
